...
type Props = {
  ...
  createApi: (fd: FormData) => Promise<Response>;
};

export default function CreateCategory({ onCreated, createApi }: Props) {
  ...
  async function submit() {
    ...
    try {
      ...
      setOpen(false);
      toast.success(`Catégorie "${form.name}" créée avec succès !`, {
        position: "bottom-right",
        autoClose: 3000,
      });
      await onCreated();
    } catch (e: any) {
      const errorMsg = e?.message ?? "Erreur création catégorie";
      setError(errorMsg);
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button className="h-10 px-4 bg-primary hover:bg-primary/90 text-primary-foreground shadow-[0_0_15px_-3px_rgba(var(--primary),0.3)] transition-all duration-200 hover:shadow-[0_0_20px_-3px_rgba(var(--primary),0.4)] rounded-lg">
          ...
          <span className="font-medium">Créer catégorie</span>
        </Button>
      </DialogTrigger>
