...
<head>
    <meta charset="utf-8">
    <title>Facture {{ $deliveryNote->number }}</title>
    <style>
        @page { margin: 0px; }
        * { box-sizing: border-box; }
        ...
        /* Big BON DE LIVRAISON title below logo */
        .facture-title-block {
    ...
    <table class="header-table">
        <tr>
            ...
            <td class="meta-cell" style="width:50%;">
                <div class="meta-row">DATE : {{ \Carbon\Carbon::parse($deliveryNote->date)->format('d / m / Y') }}</div>
                <div class="meta-row">ÉCHÉANCE : {{ $deliveryNote->due_date ? \Carbon\Carbon::parse($deliveryNote->due_date)->format('d / m / Y') : 'À RÉCEPTION' }}</div>
                <div class="meta-row facture-number">BON DE LIVRAISON N° : {{ $deliveryNote->number }}</div>
            </td>
        </tr>
    </table>
</div>

<!-- GIANT BON DE LIVRAISON TITLE -->
<div class="facture-title-block">
    <div class="facture-title">BON DE LIVRAISON</div>
</div>
                ...
                <td class="reglement-col">
                    ...
                    <div class="reglement-text">
                        ...
                        @else
                            ...
                            Veuillez indiquer le numéro de facture ({{ $deliveryNote->number }}) lors de votre paiement.
                        @endif
                    </div>
                ...
                <td class="totals-col">
                    <table class="totals-table">
                        <tr>
                            ...
                            <td class="val">{{ $formatCurrency($deliveryNote->subtotal ?? 0) }}</td>
                        </tr>
                        @if(($deliveryNote->tva ?? 0) > 0)
                        <tr>
                            <td class="lbl">TVA {{ $deliveryNote->tva }}% :</td>
                        ...
                        <tr class="grand-total">
                            ...
                            <td class="val">{{ $formatCurrency($deliveryNote->total ?? 0) }}</td>
                        </tr>
                    </table>
                </td>
            </tr>
