...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        body { font-family: 'Helvetica Neue', Helvetica, Arial, sans-serif; font-size: 11px; color: {{ $primaryColor }}; margin: 0; padding: 0; line-height: 1.5; }
        ...
        .doc-title { font-size: 32px; font-weight: bold; color: {{ $primaryColor }}; text-transform: uppercase; text-align: right; letter-spacing: 1px; margin-bottom: 5px;}
            ...
            <tr>
                ...
                <td style="padding-left: 15px;">
                    <div class="info-block">
                        ...
                        <div class="entity-name">{{ $invoice->client->name ?? '' }}</div>
                        <div class="entity-details">
                            {!! nl2br(e($invoice->client->address ?? '')) !!}<br>
                            @if($invoice->client->phone ?? null)<strong>Tél:</strong> {{ $invoice->client->phone }}<br>@endif
                            @if($invoice->client->email ?? null)<strong>Email:</strong> {{ $invoice->client->email }}@endif
                        </div>
                    </div>
                </td>
            </tr>
