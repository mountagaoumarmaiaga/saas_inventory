...
<body>
    <div class="banner">
        <table class="banner-table">
            <tr>
                <td class="b-left">
                    <div class="company-name">{{ $purchase->entreprise->name ?? 'Logistique & Transport' }}</div>
                    <div style="color: #cbd5e1;">NINEA / SIRET : {{ $purchase->entreprise->ninea ?? 'Non renseigné' }}</div>
                </td>
                <td class="b-right">
                    <div class="doc-type">BON DE COMMANDE</div>
                    <div style="font-size: 16px; font-weight: bold; margin-top: 5px;">RÉF : {{ $purchase->number }}</div>
                </td>
            </tr>
        </table>
    </div>
            ...
            <tr>
                <td class="info-col first">
                    <div class="info-box">
                        ...
                        <div class="heavy-text">{{ $purchase->entreprise->name ?? 'Entreprise' }}</div>
                        @if($purchase->entreprise->invoice_header)
                        <div style="font-size: 11px; color: #64748b; margin-top: 2px; margin-bottom: 5px; font-style: italic;">
                            {{ $purchase->entreprise->invoice_header }}
                        </div>
                        @endif
                        <div style="color: #475569;">
                            {{ $purchase->entreprise->address ?? '' }}<br>
                            Tél : {{ $purchase->entreprise->phone ?? '' }}<br>
                            Email : {{ $purchase->entreprise->email ?? '' }}
                        </div>
                    </div>
                </td>
                <td class="info-col">
                    <div class="info-box orange">
                        ...
                        <div class="heavy-text">{{ $purchase->supplier->name ?? 'Fournisseur Inconnu' }}</div>
                        <div style="color: #475569;">
                    ...
                    <div class="info-box">
                        ...
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: 900;">{{ \Carbon\Carbon::parse($purchase->order_date)->format('d/m/Y') }}</td>
                            </tr>
                            <tr>
                                <td style="padding: 4px 0; color: #64748b; font-weight: bold;">Échéance :</td>
                                <td style="text-align: right; font-weight: 900; color: #ea580c;">{{ \Carbon\Carbon::parse($purchase->order_date)->addDays(30)->format('d/m/Y') }}</td>
                            </tr>
                        </table>
                    </div>
                </td>
    ...
    <div class="summary-wrapper" style="page-break-inside: avoid;">
        ...
        <table class="summary-table">
            <tr>
                ...
                <td class="val text-right">{{ $formatCurrency(($purchase->total_amount - $purchase->tax_amount)) }}</td>
            </tr>
            @if($purchase->tax_amount > 0)
                    <tr>
                        <td>Taxes/Frais</td>
                        <td class="text-right">{{ $formatCurrency($purchase->tax_amount) }}</td>
                    </tr>
                    @endif
            <tr class="total-row">
                <td>TOTAL TTC</td>
                <td class="val text-right">{{ $formatCurrency($purchase->total_amount) }}</td>
            </tr>
        </table>
        <div class="clear"></div>
    </div>
    ...
    <div class="footer">
        @if($purchase->entreprise->invoice_footer)
            {{ $purchase->entreprise->invoice_footer }}
        @else
            <div><strong>{{ $purchase->entreprise->name ?? 'Entreprise' }}</strong></div>
