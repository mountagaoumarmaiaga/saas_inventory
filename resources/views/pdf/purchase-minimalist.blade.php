...
<body>
    <table class="header">
        <tr>
            <td style="width: 50%;">
                ...
                @endif
                <div class="company-name">{{ $purchase->entreprise->name ?? 'Studio Créatif' }}</div>
                <div style="color: #666666;">
                    {{ $purchase->entreprise->address ?? '' }}<br>
                    {{ $purchase->entreprise->phone ?? '' }}<br>
                    {{ $purchase->entreprise->email ?? '' }}
                </div>
            </td>
            <td style="width: 50%; text-align: right;">
                <div class="invoice-title">Bon de Commande</div>
                <div class="invoice-number">{{ $purchase->number }}</div>
                <div style="color: #888888; margin-top: 15px;">Date d'émission : {{ \Carbon\Carbon::parse($purchase->order_date)->format('d.m.Y') }}</div>
            </td>
        </tr>
    </table>
        ...
        <tr>
            ...
            <td style="padding-left: 40px;">
                ...
                <div style="color: #666666;">
                    ...
                    @if($purchase->supplier->email){{ $purchase->supplier->email }}@endif
                </div>
            </td>
        </tr>
    </table>
    ...
    <table class="items-table">
        ...
        <tbody>
            ...
            <tr>
                <td>
                    <div style="font-weight: bold; color: #111111;">{{ $item->product->name ?? $item->description }}</div>
                ...
                <td class="text-right" style="font-weight: bold; color: #111111;">{{ $formatCurrency($item->total_price) }}</td>
            </tr>
            @endforeach
        </tbody>
    </table>
