...
<body>
    <div class="banner">
        <table class="banner-table">
            <tr>
                <td class="b-left">
                    <div class="company-name">{{ $quote->entreprise->name ?? 'Logistique & Transport' }}</div>
                    <div style="color: #cbd5e1;">NINEA / SIRET : {{ $quote->entreprise->ninea ?? 'Non renseigné' }}</div>
                </td>
                <td class="b-right">
                    <div class="doc-type">DEVIS</div>
                    <div style="font-size: 16px; font-weight: bold; margin-top: 5px;">RÉF : {{ $quote->number }}</div>
                </td>
            </tr>
        </table>
    </div>
            ...
            <tr>
                <td class="info-col first">
                    <div class="info-box">
                        ...
                        <div class="heavy-text">{{ $quote->entreprise->name ?? 'Entreprise' }}</div>
                        @if($quote->entreprise->invoice_header)
                        <div style="font-size: 11px; color: #64748b; margin-top: 2px; margin-bottom: 5px; font-style: italic;">
                            {{ $quote->entreprise->invoice_header }}
                        </div>
                        @endif
                        <div style="color: #475569;">
                            {{ $quote->entreprise->address ?? '' }}<br>
                            Tél : {{ $quote->entreprise->phone ?? '' }}<br>
                            Email : {{ $quote->entreprise->email ?? '' }}
                        </div>
                    </div>
                </td>
                <td class="info-col">
                    <div class="info-box orange">
                        ...
                        <div class="heavy-text">{{ $quote->client->name ?? 'Client Inconnu' }}</div>
                        <div style="color: #475569;">
                    ...
                    <div class="info-box">
                        ...
                        <table style="width: 100%; border-collapse: collapse;">
                            <tr>
                                ...
                                <td style="text-align: right; font-weight: 900;">{{ \Carbon\Carbon::parse($quote->date)->format('d/m/Y') }}</td>
                            </tr>
                            <tr>
                                <td style="padding: 4px 0; color: #64748b; font-weight: bold;">Échéance :</td>
                                <td style="text-align: right; font-weight: 900; color: #ea580c;">{{ \Carbon\Carbon::parse($quote->date)->addDays(30)->format('d/m/Y') }}</td>
                            </tr>
                        </table>
                    </div>
                </td>
    ...
    <div class="summary-wrapper" style="page-break-inside: avoid;">
        ...
        <table class="summary-table">
            <tr>
                ...
                <td class="val text-right">{{ $formatCurrency($quote->subtotal) }}</td>
            </tr>
            @if($quote->tva > 0)
            <tr>
                <td>TVA ({{ $quote->tva }}%)</td>
                <td class="val text-right">{{ $formatCurrency($quote->subtotal * $quote->tva / 100) }}</td>
            </tr>
            @endif
            <tr class="total-row">
                <td>NET À PAYER TTC</td>
                <td class="val text-right">{{ $formatCurrency($quote->total) }}</td>
            </tr>
        </table>
        <div class="clear"></div>
    </div>
    ...
    <div class="footer">
        @if($quote->entreprise->invoice_footer)
            {{ $quote->entreprise->invoice_footer }}
        @else
            <div><strong>{{ $quote->entreprise->name ?? 'Entreprise' }}</strong></div>
